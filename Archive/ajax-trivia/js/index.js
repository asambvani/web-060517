let API_BASE_URL_EASY = 'https://opentdb.com/api.php?amount=10&difficulty=easy&type=boolean'
let API_BASE_URL_MEDIUM = 'https://opentdb.com/api.php?amount=10&difficulty=medium&type=boolean'
let API_BASE_URL_HARD = 'https://opentdb.com/api.php?amount=10&difficulty=hard&type=boolean'
...
$(function() {
  $('body').on('click', '#easy', function(event) {
    fetch(API_BASE_URL_EASY)
  ...
  $('body').on('click', '#medium', function(event) {
    fetch(API_BASE_URL_MEDIUM)
      .then(function(response) {
        return response.json()
      })
      .then(function(data) {
  ...
  $('body').on('click', '#hard', function(event) {
    fetch(API_BASE_URL_HARD)
      .then(function(response) {
        return response.json()
      })
      .then(function(data) {
  ...
  $('body').on('click', '.question-false', function(event) {
    const questionId = parseInt(this.dataset.id)
    const questionVal = !this.dataset.val
    const question = Question.find(questionId)
    question.answered(question.answer === questionVal)
